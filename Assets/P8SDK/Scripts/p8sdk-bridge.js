(function () {
    ...
    P8Utils.SendCallback = function (callTarget, callbackId, data, callbackType) {
        globalThis.GameGlobal.Module.SendMessage("P8SDKManager", "HandleMsgFromNative", JSON.stringify({
    ...
    */
    ...
    let sdkFunctions = {
        ...
        AdInit: function (callObj) {
            ...
            P8SDK.wxADinit(arg0, arg1, arg2, arg3);
        },
        ...
        OnActiveFunc: function (callObj) {
            P8LogSDK.onActiveFunc().then((res) => {
                // {result:"0",msg:""}
                if (res.result == 0) {
                    globalThis.GameGlobal.P8Utils.SendCallback(callObj.callTarget, callObj.callbackId, res, "success");
                } else if (res.result == 1) {
                    globalThis.GameGlobal.P8Utils.SendCallback(callObj.callTarget, callObj.callbackId, res, "fail");
                }
            });
        },

        PushLoginData: function (callObj) {
            P8LogSDK.pushLoginData(callObj.callArgs[0]).then((res) => {
                // {result:"0",data:{errorcode:200,msg:""}}
                if (res.result == 0) {
                    globalThis.GameGlobal.P8Utils.SendCallback(callObj.callTarget, callObj.callbackId, res, "success");
                } else if (res.result == 1) {
                    globalThis.GameGlobal.P8Utils.SendCallback(callObj.callTarget, callObj.callbackId, res, "fail");
        ...
        UpgradeRecord: function (callObj) {
            P8LogSDK.upGradeRecord(callObj.callArgs[0]).then((res) => {
                // {result:"0", data: {errorcode: 200, msg: "上报成功"}}
                if (res.result == 0) {
                    globalThis.GameGlobal.P8Utils.SendCallback(callObj.callTarget, callObj.callbackId, res, "success");
                } else if (res.result == 1) {
                    globalThis.GameGlobal.P8Utils.SendCallback(callObj.callTarget, callObj.callbackId, res, "fail");
        ...
        SignLog: function (callObj) {
            P8LogSDK.signLog(callObj.callArgs[0]).then((res) => {
                // {result:"0", data: {errorcode: 200, msg: "上报成功"}}
                if (res.result == 0) {
                    globalThis.GameGlobal.P8Utils.SendCallback(callObj.callTarget, callObj.callbackId, res, "success");
                } else if (res.result == 1) {
        ...
        RewardedAdLog: function (callObj) {
            P8LogSDK.wxVideoAutoLog(callObj.callArgs[0]).then((res) => {
                // {result:"0", data: {errorcode: 200, msg: "上报成功"}}
                if (res.result == 0) {
                    globalThis.GameGlobal.P8Utils.SendCallback(callObj.callTarget, callObj.callbackId, res, "success");
                } else if (res.result == 1) {
